...
async function task1 () {
  try {
    ...
    ).sort({age: "ascending"}).limit(5).toArray()
    console.log(result)
  } catch (err) {
...
async function task9 () {
  try {
    const result = await articlesCollection.find({ tags: { $in: ['super', 'tag2-a'] } }, { _id: 0 }).toArray()
    console.log(result)
  } catch (err) {
...
async function task10 () {
  try {
    const result = await studentsCollection.aggregate([
      ...
      {
        $group: {
          _id: { student_id: "$scores.student_id", name: "$name" },
      ...
      },
      {
        $project: {
          _id: 0,
          name: "$_id.name",
          worst_homework_score: "$worst_homework_score"
        }
      }
    ]).toArray()
    console.log(result)
